...
{
	CvVideoWriter * ret = cvCreateVideoWriter(file, fourcc, fps, cvSize(width, height), 1);
	printf("Return CVW %d\n",ret);
  return ret;
}


int wrap_cvWriteFrame(CvVideoWriter* writer, const IplImage* image)
{
	int ret=0;
	...
	CvVideoWriter *writer2 = cvCreateVideoWriter("/home/barufa/video_ex.mp4", CV_FOURCC('X', 'V', 'I', 'D'), 20, cvSize(640, 480));
	...
	for(;;)
        {
            ...
            if( !image )
              ...
              int ret = cvWriteFrame(writer2, image);
			  printf("Return %d\n",ret);
	...
	ret = cvWriteFrame(writer,image);
	printf("Return %d\n",ret);
	cvShowImage( "Return cvWriteFrame" , image );
